...
void handleSensorConfig()
  ...
  if (server.arg("confirm").indexOf("yes")<0)
    ...
    // display confirmation page 
     ...
     // get current accelerometer pitch from Teensy
    delay(200);
    Serial.flush();
    ...
    unsigned long starttime=millis();
      ...
      if (Serial.available()>0) 
          {
           ...
           starttime=millis();       
           if (responseString.indexOf("</PITCH>")>0)
                {
     ...
     currentConfig+=" corrected Pitch: "+ String(aircraftPitch+pitchBias);
    
    page+="<br><b>Current sensor configuration:</b><br><br>"+currentConfig+"<br>\
    <p style=\"color:black\">This procedure will configure the system\'s accelerometers, gyros and pressure sensors.<br><br>\
    <b>Requirements:</b><br><br>\
    1. Box orientation is set up properly in <a href=\"aoaconfig\">AOA Configuration</a><br>\
    2. If the aircraft is not in a level attitude enter the current pitch below.<br>\
    3. No wind condition. Preferably inside a closed hangar, no moving air\
    <br><br>\
    Configuration will take less than 1 minute.\
    </p>\
    <br>\
    <br>\
    ...
    <br><br><br>\
    <button type=\"submit\" class=\"button\">Configure Sensors</button>\
    <a href=\"/\">Cancel</a>\
    </form>";
    } else 
          {
          // configure sensors
          if (server.arg("aircraftPitch").indexOf("yes")<0)
              {
              aircraftPitch=server.arg("aircraftPitch").toFloat();
              } else aircraftPitch=0.0;     
                 
          Serial.printf("$SENSORCONFIG<AIRCRAFTPITCH>%.2f</AIRCRAFTPITCH>!",aircraftPitch);
          ...
          while (true)
            ...
            if (configResponse.indexOf("</SENSORCONFIG>") >= 0)
              {
              page+="<br><br><p>Sensors are now configured.</p>";
                    ...
                    if (configResponse.indexOf("</SENSORCONFIGERROR>") >= 0) 
                        {
                         ...
                         page+="<br><br><p>Error: Could not configure sensors. Please reboot and try again.</p>";
                         break;
                        }
            if (millis()-configStartTime>30000)
                {
                // format timeout
                page+="<br><br><p>Timeout configuring sensors. Please reboot and try again.</p>";
                break;
                }
            } // while
          }     
...
for (int curveIndex=0; curveIndex< MAX_CURVE_COEFF; curveIndex++)
            {
            boomAlphaCurve.Items[curveIndex]=stringToFloat(server.arg("boomAlphaCurveCoeff"+String(curveIndex)));
            boomAlphaCurve.curveType=1; // polynomial
            boomBetaCurve.Items[curveIndex]=stringToFloat(server.arg("boomBetaCurveCoeff"+String(curveIndex)));
            boomBetaCurve.curveType=1; // polynomial
            boomStaticCurve.Items[curveIndex]=stringToFloat(server.arg("boomStaticCurveCoeff"+String(curveIndex)));
            boomStaticCurve.curveType=1; // polynomial
            boomDynamicCurve.Items[curveIndex]=stringToFloat(server.arg("boomDynamicCurveCoeff"+String(curveIndex)));
            boomDynamicCurve.curveType=1; // polynomial
            }
// cas Curve
for (int curveIndex=0; curveIndex< MAX_CURVE_COEFF; curveIndex++)
    {
    ...
    for (int flapIndex=0; flapIndex<flapDegrees.Count; flapIndex++)
        ...
        page+="<h2> Flap Curve "+String(flapIndex+1)+" </h2>\
            ...
            // add flap position
              ...
              // boom settings  
              page+="<div class=\"form-divs flex-col-12\">\
                <label for=\"id_readBoom\">TestBoom data</label>\
              ...
              // boom curves
              String boomStyle;
              if (readBoom) boomStyle="style=\"display:block\""; else boomStyle="style=\"display:none\"";
              page+="<div class=\"form-divs flex-col-12 curvelabel boomsetting\""+boomStyle+">\
                <label>Boom Alpha Curve</label>\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomAlphaCurveCoeff0\" class=\"curve\" name=\"boomAlphaCurveCoeff0\" type=\"text\" value=\""+floatToString(boomAlphaCurve.Items[0])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>3</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomAlphaCurveCoeff1\" class=\"curve\" name=\"boomAlphaCurveCoeff1\" type=\"text\" value=\""+floatToString(boomAlphaCurve.Items[1])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>2</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomAlphaCurveCoeff2\" class=\"curve\" name=\"boomAlphaCurveCoeff2\" type=\"text\" value=\""+floatToString(boomAlphaCurve.Items[2])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup></sup>+\
              </div>\
              <div class=\"form-divs flex-col-3 boomsetting\""+boomStyle+">\
                <input id=\"id_boomAlphaCurveCoeff3\" class=\"curve\" name=\"boomAlphaCurveCoeff3\" type=\"text\" value=\""+floatToString(boomAlphaCurve.Items[3])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-12 curvelabel boomsetting\""+boomStyle+">\
                <label>Boom Beta Curve</label>\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomBetaCurveCoeff0\" class=\"curve\" name=\"boomBetaCurveCoeff0\" type=\"text\" value=\""+floatToString(boomBetaCurve.Items[0])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>3</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomBetaCurveCoeff1\" class=\"curve\" name=\"boomBetaCurveCoeff1\" type=\"text\" value=\""+floatToString(boomBetaCurve.Items[1])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>2</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomBetaCurveCoeff2\" class=\"curve\" name=\"boomBetaCurveCoeff2\" type=\"text\" value=\""+floatToString(boomBetaCurve.Items[2])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup></sup>+\
              </div>\
              <div class=\"form-divs flex-col-3 boomsetting\""+boomStyle+">\
                <input id=\"id_boomBetaCurveCoeff3\" class=\"curve\" name=\"boomBetaCurveCoeff3\" type=\"text\" value=\""+floatToString(boomBetaCurve.Items[3])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-12 curvelabel boomsetting\""+boomStyle+">\
                <label>Boom Static Curve</label>\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomStaticCurveCoeff0\" class=\"curve\" name=\"boomStaticCurveCoeff0\" type=\"text\" value=\""+floatToString(boomStaticCurve.Items[0])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>3</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomStaticCurveCoeff1\" class=\"curve\" name=\"boomStaticCurveCoeff1\" type=\"text\" value=\""+floatToString(boomStaticCurve.Items[1])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>2</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomStaticCurveCoeff2\" class=\"curve\" name=\"boomStaticCurveCoeff2\" type=\"text\" value=\""+floatToString(boomStaticCurve.Items[2])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup></sup>+\
              </div>\
              <div class=\"form-divs flex-col-3 boomsetting\""+boomStyle+">\
                <input id=\"id_boomStaticCurveCoeff3\" class=\"curve\" name=\"boomStaticCurveCoeff3\" type=\"text\" value=\""+floatToString(boomStaticCurve.Items[3])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-12 curvelabel boomsetting\""+boomStyle+">\
                <label>Boom Dynamic Curve</label>\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomDynamicCurveCoeff0\" class=\"curve\" name=\"boomDynamicCurveCoeff0\" type=\"text\" value=\""+floatToString(boomDynamicCurve.Items[0])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>3</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomDynamicCurveCoeff1\" class=\"curve\" name=\"boomDynamicCurveCoeff1\" type=\"text\" value=\""+floatToString(boomDynamicCurve.Items[1])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup>2</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2 boomsetting\""+boomStyle+">\
                <input id=\"id_boomDynamicCurveCoeff2\" class=\"curve\" name=\"boomDynamicCurveCoeff2\" type=\"text\" value=\""+floatToString(boomDynamicCurve.Items[2])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1 boomsetting\""+boomStyle+">\
                *X<sup></sup>+\
              </div>\
              <div class=\"form-divs flex-col-3 boomsetting\""+boomStyle+">\
                <input id=\"id_boomDynamicCurveCoeff3\" class=\"curve\" name=\"boomDynamicCurveCoeff3\" type=\"text\" value=\""+floatToString(boomDynamicCurve.Items[3])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-12 curvelabel\">\
                <label>Calibrated Airspeed Curve</label>\
              </div>\
              <div class=\"form-divs flex-col-2\">\
                <input id=\"id_casCurveCoeff0\" class=\"curve\" name=\"casCurveCoeff0\" type=\"text\" value=\""+floatToString(casCurve.Items[0])+"\"/>\
              </div>\
              <div class=\"form-divs flex-col-1\">\
                *X<sup>3</sup>+\
              </div>\
              <div class=\"form-divs flex-col-2\">\
            ...
            //sensor biases                      
              page+="<!--\
              <div class=\"form-divs flex-col-12\">\
                <label for=\"id_pStaticBias\">PStatic Bias</label>\
                <input id=\"id_pStaticBias\" name=\"pStaticBias\" type=\"text\" value=\""+String(pStaticBias)+"\"/>\
              </div>\
              -->\
              <div class=\"form-divs flex-col-12\">\
              ...
              // loadLimit    
              page+="<div class=\"form-divs flex-col-12\">\
                <label for=\"id_loadLimit\">Aircraft load factor limit</label>\
                <select id=\"id_loadLimit\" name=\"loadLimit\">\
                  <option value=\"6.00\""; if (String(loadLimit)=="6.00") page+=" selected"; page+=">Aerobatic Category +6.0G</option>\
                  <option value=\"4.40\""; if (String(loadLimit)=="4.40") page+=" selected"; page+=">Utility Category +4.4G</option>\
                  <option value=\"3.80\""; if (String(loadLimit)=="3.80") page+=" selected"; page+=">Standard Category +3.8G</option>\
                  <option value=\"2.50\""; if (String(loadLimit)=="2.50") page+=" selected"; page+=">G Limit Test +2.5G</option>\
                </select>\
              </div>";
...
// javascript code
        page+="<script>";
// hide boom settings when boom data is disabled         
        page+="document.getElementById(\'id_readBoom\').onchange = function() {\
          if (document.getElementById(\'id_readBoom\').value==1)\
          {\
          [].forEach.call(document.querySelectorAll(\'.boomsetting\'), function (el) {el.style.display = \'block\';});\
          } else\
              {\
